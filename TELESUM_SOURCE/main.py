...
app.add_middleware(SessionMiddleware, secret_key="your_default_secret_key")
...
async def new_message_handler(event):
    # Проверка, что сообщение пришло в личный чат
    if event.is_private:
        sender = await event.get_sender()
        sender_message = event.message.message
        sender_name = f"{sender.first_name or ''} {sender.last_name or ''}".strip()
        logger.info(f"Новое сообщение от {sender_name} {sender.last_name}: {sender_message}")

        # Отправка сообщения на обработку OpenAI
        response_text = summarize_text(sender_message)

        # Отправка ответа обратно в чат
        await event.reply(response_text)
        logger.info(f"Ответ отправлен {sender_name} {sender.last_name}: {response_text}")

# Запуск Telethon-client
async def start_telegram_client():
    try:
        await client.start()
        logger.info("Телеграмм клиент запущен")
        await client.run_until_disconnected()
    except Exception as e:
        logger.error(f"Ошибка при работе телеграмм клиента: {e}")

# Фоновая задача для запуска Telethon-client
def run_telegram_client_in_thread():
    loop = asyncio.new_event_loop()  # Создаем новый событийный цикл
    asyncio.set_event_loop(loop)     # Устанавливаем его как текущий
    loop.run_until_complete(start_telegram_client())  # Запускаем Telethon-client

@app.on_event("startup")
async def startup_event():
    # Запускаем Telethon-client в отдельном потоке
    thread = Thread(target=run_telegram_client_in_thread, daemon=True)
    thread.start()
